...
type Backend interface {
	// Retrieval read-only queries for software trees  检索软件树的只读查询
	...
	// Retrieval read-only queries for evidence trees  检索证据树的只读查询
	...
	Criticalityscore(ctx context.Context, certifyCriticalityscoreSpec *model.CertifyCriticalityscoreSpec) ([]*model.CertifyCriticalityscore, error)
	...
	// Mutations for software trees (read-write queries)  软件树的突变（读写查询）
